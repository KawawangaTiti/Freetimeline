...
(function () {
  ...
  let _mufLastTs = 0;
  (function tick(ts) {
    // Idle-CPU gate (Phase 6 / UE-14): this loop only mirrors data edits into
...
const M = {
  ...
  _continuityEditor(top) {
    ...
    this.body().innerHTML =
      '<div style="margin-bottom:14px;font-size:13px;color:#666;line-height:1.6">' +
        'Continuities let one timeline hold <b>competing chronologies</b> — canon, fan theories, alternate readings. ' +
      ...
      '<div style="border-top:1px solid #eee;padding-top:14px;margin-top:8px">' +
        '<div style="display:flex;gap:8px;align-items:center">' +
          '<input id="ct-new-name" placeholder="New continuity, e.g. “Anthology theory”..." style="flex:1;padding:5px 9px;border:1px solid #d6d6d6;border-radius:5px;font-size:13px;font-family:inherit">' +
...
/* =====================================================
   ...
   ===================================================== */
/* ---- Continuity editor handlers (WS4) ---- */
...
const Store = {
  ...
  saveHTML() {
    ...
    Promise.all([fetch(location.href).then(r => r.text()), Store._mapExportPromise()])
      .then(([src, m]) => {
        ...
        Store._downloadBlob(Store._buildHTMLExport(src, m.du, m.imgs), 'text/html', 'timeline_' + date + '.html');
  ...
  _saveHTMLFallback() {
    Store._mapExportPromise().then(m => {
      const date = new Date().toISOString().slice(0, 10);
      Store._downloadBlob(Store._buildHTMLExport(document.documentElement.outerHTML, m.du, m.imgs), 'text/html', 'timeline_' + date + '.html');
      notify('Timeline saved as HTML \u2713', 'success');
    });
  },
  ...
  /* ---- Level 0: share a timeline by link (no server; maps not included) ---- */
  shareLink() {
    if (!window.ftShare) { notify('Sharing needs a modern browser.', 'error'); return; }
    var payload = { _ft: 'universe-share1',
      universes: S.universes, events: S.events, connections: S.connections,
      characters: S.characters, categories: S.categories, affiliations: S.affiliations,
  ...
  loadShared(raw) {
    var d = (typeof ftValidateImport === 'function') ? ftValidateImport(raw, { kind: 'universe' }) : raw;
    ftConfirmGate('Open this shared timeline? It will REPLACE your current data.', function () {
      S.universes = d.universes || []; S.events = d.events || []; S.connections = d.connections || [];
      S.characters = d.characters || []; S.categories = d.categories || {}; S.affiliations = d.affiliations || [];
  ...
  cloudExport() {
    var name = (S.universes && S.universes[0] && S.universes[0].name) || 'My timeline';
    return { app: 'universe', title: String(name).slice(0, 120), data: {
      _ft: 'universe-share1', universes: S.universes, events: S.events, connections: S.connections,
      characters: S.characters, categories: S.categories, affiliations: S.affiliations,
...
/* DOC-004: label <-> internal-key mapping differs across the two apps (and is
   cross-wired). Here in Universe the connection-map's USER LABEL is 'Map' but its
   internal view key is 'connections' (id tab-connections). In Biography it's the
   inverse: label 'Relationships' but key 'map'. So Universe 'connections' ===
   Biography 'map'. Do NOT rename these keys — saved view-state hashes depend on
   them; just keep this mapping in mind when porting a connection-map bugfix. */
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  /* Frame the timeline on the actual data. The default range is geological
     (200,000 BC – 20,000 AD), leaving events as lost specks with nothing to
...
function updateAllClearBtn() {
  ...
  if (!btn) return;
  var anyActive = _catFilter || _statusFilter || _tagFilter || _toneFilter || _placeFilter || _continuityFilter || _charFilterIds.length > 0 || _searchText.length > 0;
...
(function () {
  ...
  function restore(snap) {
    ...
    try {
      ...
      if (snap.places)               S.places               = snap.places;
      if (snap.maps !== undefined)   S.maps                 = snap.maps;
      if (snap.mapMeta)              S.mapMeta              = snap.mapMeta;
      if (snap.currentMapId)         S.currentMapId         = snap.currentMapId;
      if (snap.mapSketchIcons)       S.mapSketchIcons       = snap.mapSketchIcons;
      if (snap.mapSketchIconsByMap)  S.mapSketchIconsByMap  = snap.mapSketchIconsByMap;
  ...
  function anyFilterActive() {
    return !!(
      ...
      (typeof _continuityFilter !== 'undefined' && _continuityFilter) ||
  ...
  function clearAllFilters() {
    ...
    try { if (typeof _continuityFilter !== 'undefined') _continuityFilter = null; } catch (_) {}
    try { if (typeof _charFilterIds !== 'undefined')  _charFilterIds.length = 0; } catch (_) {}
    ['updateCatFilterBar','updateStatusFilterBar','updateTagFilterBar','updateToneFilterBar','updatePlaceFilterBar','updateContinuityFilterBar','updateCharFilterSelect']
      .forEach(function (fn) { try { if (typeof window[fn] === 'function') window[fn](); } catch (_) {} });
    if (typeof render === 'function') render();
